...
import { CombatVfxCanvasRef } from '@/src/features/combat/components/CombatVfxCanvas';
...
export default function App() {
  ...
  const vfxRef = useRef<CombatVfxCanvasRef>(null);
  ...
  // ── Sandbox Mode ────────────────────────────────────────────────────────────
  const [sandboxTokens, setSandboxTokens] = useState<{ A: string; B: string } | null>(() => {
    const sess = apiService.loadSession();
    return sess?.sandboxTokens ?? null;
  });
    ...
    try {
      ...
      if (phase === 'draft' && appState !== 'createMatch' && appState !== 'lobby') {
        ...
        apiService.getMapCover(session.roomId, data.gameState.mapId)
          .then(coverData => setMapCoverConfig(coverData || {}))
          .catch(() => {});
        setTimeout(() => setPhaseTransitioning(null), 200);
      }
    } catch {
  ...
  const handleUnitClick = useCallback(async (unitId: string) => {
    ...
    if (targetMode === 'shoot' || targetMode === 'suppress') {
      ...
      setTargetMode(null);
      const fromPos = { x: atk.x, y: atk.y };
      const toPos = { x: target.x, y: target.y };
      const atkRole = atk.className.toLowerCase();

      if (targetMode === 'suppress') {
        vfxRef.current?.triggerShot({
          from: fromPos,
          to: toPos,
          outcome: 'hit',
          role: atkRole,
        ...
        const res = await withError(() => apiService.shootUnit(session.roomId, session.playerToken, atk.id, unitId, distPenalty));
        if (res) {
          const outcome = res.shotResult?.isCrit ? 'crit' : res.shotResult?.hit ? 'hit' : 'miss';
          vfxRef.current?.triggerShot({
            from: fromPos,
            to: toPos,
            outcome,
            role: atkRole,
            damage: res.shotResult?.damage,
          });
        }
      }
  ...
  const resolveGuardShot = useCallback(async (accept: boolean) => {
    ...
    const guard = gameState?.units[guardShotDecision.guardUnitId];
    const target = gameState?.units[guardShotDecision.targetUnitId];

    if (accept && guard && target) {
      vfxRef.current?.triggerShot({
        from: { x: guard.x, y: guard.y },
        to: { x: target.x, y: target.y },
        role: guard.className.toLowerCase(),
      });
    }
  ...
  }, [guardShotDecision, session, gameState]);
  ...
  const handleCanvasClick2D = useCallback((e: React.MouseEvent) => {
    ...
    if (targetMode === 'grenade' && selectedUnit) {
      const fromPos = { x: selectedUnit.x, y: selectedUnit.y };
      ...
      setTimeout(() => {
        vfxRef.current?.triggerExplosion({ x: targetX, y: targetY, radiusPx: 80 });
      }, 250);
      withError(() => apiService.throwGrenade(session!.roomId, session!.playerToken, selectedUnit.id, { gx, gy }, 0));
      return;
    }

    if (targetMode === 'smoke' && selectedUnit) {
      const fromPos = { x: selectedUnit.x, y: selectedUnit.y };
      const targetX = gx * CELL_SIZE + CELL_SIZE / 2;
      const targetY = gy * CELL_SIZE + CELL_SIZE / 2;
      setTargetMode(null);
      ...
      setTimeout(() => {
        vfxRef.current?.triggerExplosion({ x: targetX, y: targetY, radiusPx: 60 });
      }, 250);
      withError(() => apiService.smokeGrenade(session!.roomId, session!.playerToken, selectedUnit.id, { gx, gy }));
      return;
    }
  ...
  }, [isPanning, zoom, camera, targetMode, moveSubMode, moveManualPath, reachableMap, facingMode, handleCellClick, handleSetFacingClick, selectedUnit, session]);
      ...
      <div className="fixed top-4 left-4 z-[9999] flex items-center gap-2">
        <button
          onClick={() => {
            ...
            const updated = { ...session, playerTeam: nextTeam, playerToken: nextToken, sandboxTokens };
            setSession(updated);
            apiService.saveSession(updated);
          }}
          className={cn(
            "group flex items-center gap-3 px-4 py-2.5 rounded-xl font-mono text-xs font-black uppercase tracking-wider transition-all shadow-2xl border active:scale-95 backdrop-blur-md",
            isA
              ? "bg-blue-600/90 hover:bg-blue-500 text-white border-blue-400/50 shadow-blue-900/40"
              : "bg-red-600/90 hover:bg-red-500 text-white border-red-400/50 shadow-red-900/40"
          )}
          title="Clique para alternar entre Jogador A e Jogador B no Sandbox"
        >
          <div className="w-2.5 h-2.5 rounded-full bg-amber-400 animate-ping" />
          <span className="flex items-center gap-1.5">
            <Users size={15} />
            SANDBOX: <span className="underline decoration-2 underline-offset-2">{isA ? 'EQUIPE A (EUA)' : 'EQUIPE B (TERROR)'}</span>
          </span>
          <span className="text-[10px] bg-black/40 px-2 py-0.5 rounded border border-white/20 ml-1 text-amber-300">
            Trocar ➔
          </span>
        </button>
      </div>
  ...
  if ((appState === 'deploy' || appState === 'waiting') && session) {
    return (
      <div className="min-h-screen w-full bg-neutral-950 flex flex-col relative">
        <SandboxToggle />
        <DeployScreen
          roomId={session.roomId}
          playerToken={session.playerToken}
