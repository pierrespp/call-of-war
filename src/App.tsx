...
export default function App() {
  ...
  const [movePendingPath, setMovePendingPath] = useState<PathStep[] | null>(null);  // shown in confirm modal
  ...
  const [targetMode, setTargetMode] = useState<"move" | "shoot" | null>(null);
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape") {
        if (movePendingPath) { setMovePendingPath(null); return; }
      ...
      if (e.key === "Enter" && movePendingPath && !isMoving) {
        e.preventDefault();
        confirmAutoMove();
      }
    };
  ...
  }, [appState, movePendingPath, facingMode, targetMode, selectedUnitId, isMoving]);
  ...
  // Decoupled from gameState polling so the confirm modal is not closed by
  // background room-state updates (this used to make the modal disappear
  ...
  // (gameState, cover config). This effect MUST NOT clear movePendingPath /
  // moveManualPath — otherwise polling would dismiss the open confirm modal.
  useEffect(() => {
  ...
  const handleCanvasClick = async (e: React.MouseEvent) => {
    ...
    // Auto preview: clicking a reachable cell opens confirm modal with shortest path
    const key = `${gx},${gy}`;
    ...
    if (!path) return;
    setMovePendingPath(path);
  };

  const confirmAutoMove = () => {
    if (movePendingPath) sendMovePath(movePendingPath);
  };
  const switchToManualMove = () => {
    if (!movePendingPath) return;
    const start = movePendingPath[0];
    setMoveManualPath([start]);
    setMovePendingPath(null);
  ...
  const handleUnitClick = async (e: React.MouseEvent, unit: Unit) => {
    ...
    if (targetMode === "shoot" && selectedUnitId && gameState) {
      ...
      if (weapon) {
        if (weapon.range === "Curto" && distanceMeters > SCALE.ALCANCE_CURTO) distancePenalty = Math.max(0, Math.floor((distanceMeters - SCALE.ALCANCE_CURTO) * 2));
        else if (weapon.range === "Médio" && distanceMeters > SCALE.ALCANCE_MEDIO) distancePenalty = Math.max(0, Math.floor((distanceMeters - SCALE.ALCANCE_MEDIO) * 1));
        else if (weapon.range === "Longo" && distanceMeters > SCALE.ALCANCE_LONGO) distancePenalty = Math.max(0, Math.floor((distanceMeters - SCALE.ALCANCE_LONGO) * 0.5));
      }
      let hitRate = (CLASSES[source.className]?.hit ?? 60) - distancePenalty;
  ...
  const computeGuardShotInfo = (pending: PendingGuardShot) => {
    ...
    let hitRate = (CLASSES[guard.className]?.hit ?? 60) - distancePenalty - 10; // -10 from guard
    if (coverLevel === "half") hitRate -= 20;
    if (coverLevel === "full") hitRate -= 40;
    ...
    <div className="flex h-screen bg-neutral-900 text-neutral-200 overflow-hidden font-sans">
      ...
      >
        ...
        >
          ...
          {targetMode === "move" && moveReachable && moveHoverCell && !moveManualPath && !movePendingPath && selectedUnit && (() => {
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  targetMode === "shoot" && isEnemy && "animate-pulse ring-4 ring-red-500",
      ...
      {/* Move Confirm Modal (auto path picked) */}
      {movePendingPath && (
        <MoveConfirmModal
          pendingPath={movePendingPath}
          costMeters={pathCostMeters(movePendingPath, mapCoverConfig)}
          onConfirmAuto={confirmAutoMove}
          onSwitchManual={switchToManualMove}
          onCancel={() => setMovePendingPath(null)}
        />
      )}
