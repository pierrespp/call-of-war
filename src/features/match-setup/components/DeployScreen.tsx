...
import { CELL_SIZE, CLASSES } from '@/src/core/data/constants';
import { useMaps } from '@/src/core/contexts/MapContext';
import { apiService, RoomStateResponse } from '@/src/core/services/apiService';
import { DeployZone, DraftUnit, MapCoverData } from '@/src/types/game';
import { Check, X, ArrowLeft } from "lucide-react";
import { getImageUrl } from '@/src/lib/utils';
import { BUILTIN_TILESETS } from '@/src/core/data/tilesets';
import { createTileLookup } from '@/src/utils/tilesetUtils';
import { MapCellTile, MapTileData } from '@/src/types/tileset';
...
export function DeployScreen({ roomId, playerToken, playerTeam, state, onLeave, onRefreshState }: Props) {
  const { maps } = useMaps();
  ...
  const [tilesData, setTilesData] = useState<MapTileData>({});
  const tileLookup = useMemo(() => createTileLookup(BUILTIN_TILESETS), []);
  const tileCanvasRef = useRef<HTMLCanvasElement>(null);
  const tileImagesCache = useRef<Record<string, HTMLImageElement>>({});
  ...
  useEffect(() => {
    ...
    if (mapId) {
      fetch(`/api/maps/${mapId}/tiles`)
        .then(res => res.ok ? res.json() : {})
        .then(data => setTilesData(data || {}))
        .catch(() => setTilesData({}));
    }
  }, [roomId, mapId]);

  const mapW = (mapInfo?.gridWidth || 20) * CELL_SIZE;
  const mapH = (mapInfo?.gridHeight || 20) * CELL_SIZE;

  // Pré-renderiza os tiles no canvas quando tilesData mudar ou imagens carregarem
  useEffect(() => {
    ...
    const renderAllTiles = () => {
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      for (const [key, cellTile] of tileEntries) {
        const tile = cellTile as MapCellTile;
        if (!tile) continue;
        const tileDef = tileLookup[tile.tileId];
        if (!tileDef) continue;
        ...
        const img = tileImagesCache.current[tile.tileId];
        if (img && img.complete) {
          ctx.save();
          if (tile.rotation) {
    ...
    tileEntries.forEach(([_, cellTile]) => {
      const tile = cellTile as MapCellTile;
      if (!tile) return;
      const tileDef = tileLookup[tile.tileId];
      if (!tileDef) return;
    ...
    renderAllTiles();
  }, [tilesData, tileLookup, mapW, mapH]);

  const lastTeamRef = useRef<string | null>(null);

  useEffect(() => {
    if (playerTeam !== lastTeamRef.current) {
      setPositions(state.deploy.positions[playerTeam] || {});
      lastTeamRef.current = playerTeam;
  ...
  useEffect(() => {
    ...
    if (myZones.length === 1 && myZones[0]?.id) {
  ...
  const placeOrSwap = async (gx: number, gy: number) => {
    ...
    const occupiedBy = Object.entries(positions).find(([, p]: [string, { gx: number; gy: number }]) => p.gx === gx && p.gy === gy);
  ...
  const handleToggleReady = async () => {
    ...
    setErrorMsg(null);
    try {
      await apiService.setDeployReady(roomId, playerToken, !myReady);
  ...
  const unitsByPosition = useMemo(() => {
    ...
    for (const [uid, p] of Object.entries(positions)) {
      const pos = p as { gx: number; gy: number };
      m.set(`${pos.gx},${pos.gy}`, uid);
    }
    return m;
  }, [positions]);
  ...
  return (
    <div className="flex h-full w-full text-neutral-200 overflow-hidden">
      {/* Sidebar */}
      <div className="w-96 glass-panel-dark border-r border-white/5 flex flex-col shadow-2xl z-10 backdrop-blur-2xl">
        <div className="p-5 border-b border-neutral-700">
          <button onClick={onLeave} className="btn-tactical flex items-center gap-2 text-neutral-400 hover:text-white transition-colors mb-4 text-[10px] font-black uppercase tracking-widest bg-white/5 px-3 py-1.5 rounded-lg border border-white/5">
      ...
      >
        ...
        >
          ...
          {/* Placed units */}
          {Object.entries(positions).map(([uid, pRaw]) => {
            const p = pRaw as { gx: number; gy: number };
