import { CoverType, DeployZone } from '@/src/types/game';
...
export function findPathToClosestReachable(
  startG: { gx: number; gy: number },
  tgtG: { gx: number; gy: number },
  cover: Record<string, CoverType | string>,
  movementBudget: number,
  units: import('@/src/types/game').Unit[]
): PathStep[] | null {
  // 1. Get all reachable cells within the budget
  const occupied = new Set(units.filter(u => u.hp > 0).map(u => `${Math.floor(u.x/50)},${Math.floor(u.y/50)}`));
  const startKey = `${startG.gx},${startG.gy}`;
  occupied.delete(startKey); // Source is not blocking self

  const queue: Array<{gx: number, gy: number, cost: number, path: PathStep[]}> = [
    { gx: startG.gx, gy: startG.gy, cost: 0, path: [{ gx: startG.gx, gy: startG.gy }] }
  ];
  const visited = new Map<string, number>();
  let bestCell = queue[0];
  let minDistanceToTarget = Math.sqrt(Math.pow(tgtG.gx - startG.gx, 2) + Math.pow(tgtG.gy - startG.gy, 2));
  ...
  while (queue.length > 0) {
    ...
    // Track the cell that gets us closest to the target
    const dist = Math.sqrt(Math.pow(tgtG.gx - cur.gx, 2) + Math.pow(tgtG.gy - cur.gy, 2));
    if (dist < minDistanceToTarget) {
      minDistanceToTarget = dist;
      bestCell = cur;
    }

    if (cur.cost >= movementBudget) continue;
    ...
    for (const n of neighbours) {
      ...
      const t = cover[nk];
      const isSolid = t === 'wall' || t === 'half' || t === 'full' || t === 'doorClose';
      if (isSolid) continue;
      
      const stepCost = (t === 'water' || t === 'window') ? WATER_CELL_COST : NORMAL_CELL_COST;
      const newCost = cur.cost + stepCost;
      
      if (newCost <= movementBudget + 0.1) {
        queue.push({ gx: n.gx, gy: n.gy, cost: newCost, path: [...cur.path, n] });
      }
    }
  }

  return bestCell.path.length > 1 ? bestCell.path : null;
}
