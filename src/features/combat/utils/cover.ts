import { CELL_SIZE } from "../../../core/data/constants";
...
export interface ShotCoverResult {
  ...
  /** Silent Run: Car alarm cells crossed by the line. */
  alarmCells?: string[];
}
...
export function hasLineOfSight(
  ...
  const ax = Math.floor(pixelAx / CELL_SIZE);
  const ay = Math.floor(pixelAy / CELL_SIZE);
  const tx = Math.floor(pixelTx / CELL_SIZE);
  const ty = Math.floor(pixelTy / CELL_SIZE);
  ...
  while (true) {
    ...
    if (!isAttackerCell && !isTargetCell) {
      const key = `${x},${y}`;
      const c = cover[key] as CoverType | undefined;
      if (c === "wall" || c === "doorClose") return false;
    }
