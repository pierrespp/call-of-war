...
import { MapTileData, MapCellTile } from '@/src/types/tileset';
import { BUILTIN_TILESETS } from '@/src/core/data/tilesets';
import { createTileLookup } from '@/src/utils/tilesetUtils';
import { CombatVfxCanvas, CombatVfxCanvasRef } from './CombatVfxCanvas';
...
interface BattleCanvas2DProps {
  ...
  vfxRef?: React.RefObject<CombatVfxCanvasRef>;
}
...
export const BattleCanvas2D: React.FC<BattleCanvas2DProps> = ({
  ...
  onMouseMove, onMouseUp, onMouseLeave, onContextMenu, setCanvasRef, vfxRef,
}) => {
  const { maps } = useMaps();
  ...
  const [tilesData, setTilesData] = React.useState<MapTileData>({});
  const tileLookup = React.useMemo(() => createTileLookup(BUILTIN_TILESETS), []);
  const tileCanvasRef = React.useRef<HTMLCanvasElement>(null);
  const tileImagesCache = React.useRef<Record<string, HTMLImageElement>>({});

  React.useEffect(() => {
    if (!gameState.mapId) return;
    fetch(`/api/maps/${gameState.mapId}/tiles`)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Combat VFX Canvas (Trajetórias balísticas, Muzzle Flash, Sangue, Faíscas e Explosões) */}
        <CombatVfxCanvas ref={vfxRef} width={mapW} height={mapH} />
      </div>
    </div>
  );
};
