...
import { CLASSES, WEAPONS, ARMORS, SKILLS, ATTACHMENTS } from '@/src/core/data/constants';
...
export function SoldiersInfoMenu({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="w-full text-neutral-200 p-4 md:p-8 relative z-10">
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="flex justify-between items-center glass-panel p-8 rounded-3xl border border-white/5 shadow-2xl">
          <div>
        ...
        <div className="space-y-6">
               ...
               <div key={faction}>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {Object.values(CLASSES).filter(c => c.faction === faction).map(c => (
                    <div key={c.id} className="glass-panel-dark border border-white/5 p-4 rounded-2xl flex gap-4 hover:border-indigo-500/30 transition-all group">
                      <div className="w-16 h-16 bg-neutral-900 rounded-lg flex items-center justify-center shrink-0 border border-neutral-700 bg-cover bg-center" style={{ backgroundImage: `url("${getRoleImage(c.id)}")` }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.values(WEAPONS).map(w => (
              <div key={w.name} className="glass-panel-dark border border-white/5 p-4 rounded-2xl hover:border-red-500/30 transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.values(ARMORS).map(a => (
              <div key={a.name} className="glass-panel-dark border border-white/5 p-5 rounded-2xl flex justify-between items-center hover:border-emerald-500/30 transition-all">
                <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.values(SKILLS).map((s, idx) => (
              <div key={`${s.name}-${idx}`} className="bg-indigo-900/20 border border-indigo-800 p-4 rounded-xl">
                 <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <p className="text-xs font-bold text-neutral-500 mt-1">Exclusivo: {Array.isArray(s.classRequired) ? s.classRequired.join(", ") : s.classRequired}</p>
                  </div>
            ...
            {Object.values(ATTACHMENTS).map((a, idx) => (
              <div key={`${a.name}-${idx}`} className="bg-neutral-800 border border-neutral-700 p-4 rounded-xl">
                 <div className="flex justify-between items-start mb-2">
                  <div>
