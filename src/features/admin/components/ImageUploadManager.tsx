...
import { useStorage } from '@/src/core/hooks/useStorage';

export const ImageUploadManager = ({ onUploadSuccess }: { onUploadSuccess?: (url: string, type: string) => void }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      let finalUrl = '';
      if (uploadType === 'role') {
        finalUrl = await uploadRoleImage(selectedFile, itemName);
      } else if (uploadType === 'map') {
        finalUrl = await uploadMapImage(selectedFile, itemName);
      } else {
        const result = await uploadCustomToken(selectedFile, itemName);
        finalUrl = result.url;
      }
      setUploadSuccess(true);
      setSelectedFile(null);
      setItemName('');
      if (onUploadSuccess) onUploadSuccess(finalUrl, uploadType);
    } catch (err) {
    ...
    <div className="bg-neutral-900 border border-neutral-700 rounded-lg p-6 max-w-md">
      ...
      <div className="space-y-4">
        <div>
          ...
          >
            <option value="role">Token de Classe (Role)</option>
      ...
      <div className="mt-6 pt-6 border-t border-neutral-700">
        <p className="text-xs text-neutral-500">
          As imagens serão salvas no Cloudinary (se configurado) ou no Firebase Storage, e os registros serão salvos no Firestore.
        </p>
      </div>
    </div>
  );
