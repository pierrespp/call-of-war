...
export function CreateMatchMenu({ roomId, playerToken, playerTeam, state, onBack }: Props) {
  ...
  // Sync local state from polled server state — only when the server's value differs
  // from what we last sent (so we don't clobber the user mid-typing).
  useEffect(() => {
    if (!state) return;
    const serverTeam = state.draft.teams[playerTeam] || [];
    const serverJson = JSON.stringify(serverTeam);
    if (serverJson !== lastSentRef.current && units.length === 0) {
      // First-load hydrate or reset after server-side change
      setUnits(serverTeam);
      lastSentRef.current = serverJson;
    }
    if (state.draft.selectedMap !== selectedMap) setSelectedMap(state.draft.selectedMap);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state?.draft.selectedMap]);

  // Hydrate once when state first arrives (regardless of units length)
  const hydratedRef = useRef(false);
  useEffect(() => {
    if (!state || hydratedRef.current) return;
    const serverTeam = state.draft.teams[playerTeam] || [];
    setUnits(serverTeam);
    lastSentRef.current = JSON.stringify(serverTeam);
