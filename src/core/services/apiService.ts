import { GameState, MapCoverData, PendingGuardShot, DraftUnit, RoomPhase, DeployZone, GameMode, Difficulty } from '@/src/types/game';
...
export interface RoomStateResponse {
  ...
  phase: RoomPhase;
  players: { A?: { name: string }; A2?: { name: string }; A3?: { name: string }; A4?: { name: string }; B?: { name: string }; };
  draft: {
    selectedMap: string;
    gameMode?: GameMode;
...
export const apiService = {
  async createRoom(playerName: string, gameMode?: string) {
  ...
  async setDraftPveConfig(roomId: string, playerToken: string, config: { gameMode?: string, difficulty?: string, pveZombieCount?: number, pveTeamName?: string, pveTeamUnits?: DraftUnit[] }) {
    return request<{ success: boolean }>(`/rooms/${roomId}/draft/pve-config`, {
      method: "POST",
      body: JSON.stringify({ playerToken, ...config }),
    });
  },
  ...
  async shootUnit(
    ...
    return request<{ success: boolean; gameState: GameState; shotResult?: { hit: boolean; isCrit: boolean; damage: number } }>(`/rooms/${roomId}/shoot`, {
      method: "POST",
  ...
  async suppressUnit(roomId: string, playerToken: string, attackerId: string, targetId: string) {
    return request<{ success: boolean; gameState: GameState }>(`/rooms/${roomId}/suppress`, {
      method: "POST",
      body: JSON.stringify({ playerToken, attackerId, targetId }),
    });
  },
  async hailOfBullets(roomId: string, playerToken: string, attackerId: string, targetId: string) {
    return request<{ success: boolean; gameState: GameState }>(`/rooms/${roomId}/hail-of-bullets`, {
      method: "POST",
      body: JSON.stringify({ playerToken, attackerId, targetId }),
    });
  },
  async reviveUnit(roomId: string, playerToken: string, healerId: string, targetId: string) {
    return request<{ success: boolean; gameState: GameState }>(`/rooms/${roomId}/revive`, {
      method: "POST",
      body: JSON.stringify({ playerToken, healerId, targetId }),
    });
  },
  async smokeGrenade(roomId: string, playerToken: string, attackerId: string, targetCell: { gx: number; gy: number }) {
    return request<{ success: boolean; gameState: GameState }>(`/rooms/${roomId}/smoke-grenade`, {
      method: "POST",
      body: JSON.stringify({ playerToken, attackerId, targetCell }),
    });
  },
  ...
  async throwGrenade(roomId: string, playerToken: string, attackerId: string, targetCell: { gx: number, gy: number }, distancePenalty: number) {
    return request<{ success: boolean; gameState: GameState }>(`/rooms/${roomId}/grenade`, {
      method: "POST", body: JSON.stringify({ playerToken, attackerId, targetCell, distancePenalty }),
    });
  },
  ...
  async spawnZombie(roomId: string, playerToken: string, type: string, gx: number, gy: number) {
    return request<{ success: boolean; roomState: RoomStateResponse }>(`/rooms/${roomId}/pve/spawn`, {
      method: "POST", body: JSON.stringify({ playerToken, type, gx, gy }),
    });
  },
  async updatePveConfig(roomId: string, playerToken: string, config: any) {
    return request<{ success: boolean; roomState: RoomStateResponse }>(`/rooms/${roomId}/pve/config`, {
      method: "POST", body: JSON.stringify({ playerToken, config }),
    });
  },
