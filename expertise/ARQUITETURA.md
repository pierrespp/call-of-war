...
O desenvolvimento deste VTT é auxiliado por um sistema de Personas (Agentes IA). As instruções para o comportamento esperado do assistente de IA estão divididas em perfis de especialidade na pasta `/agents/`:
- `GameDevExpert.md`: Padrões de código React/TypeScript.
- `UIUXMaster.md`: Estética, animações e refinamento visual premium.
- `BalanceAnalyst.md`: Matemática do jogo (TTK, orçamentos de pontos).
- `QATester.md`: Testes de borda e falhas de sincronização.
- `GameDesigner.md`: Foco na UX e em como a mecânica "parece" ao jogador.
- `SecuritySpecialist.md`: Validações de Backend e Firebase.

O fluxo de trabalho padrão exige consultar o `AGENTS.md` na raiz para saber qual persona invocar de acordo com o desafio arquitetural.
